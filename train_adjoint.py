...
def operator_inference(Q_train, t_train, Q_test, t_test, r, split_ratio_validation=.1, use_val=True):
    ...
    A_opinf_6, H_opinf_6, regularizer_6, par_tsvd_6, loss_min_6 = \
        optimal_opinf(Q_train_, t_train, t_test, 'ord6', use_val, valid_ratio=split_ratio_validation, Q_test_=Q_test_)#, M=np.max(np.abs(Q_train_))*10, T=t[-1],)
    
    ##### result by order='ord2'
    A_opinf_2, H_opinf_2, regularizer_2, par_tsvd_2, loss_min_2 = \
        optimal_opinf(Q_train_, t_train, t_test, 'ord2', use_val, valid_ratio=split_ratio_validation, Q_test_=Q_test_)#, M=np.max(np.abs(Q_train_))*10, T=t[-1],)
    ...
    return A_opinf, H_opinf, Q_train_, svdvals, regularizer, par_tsvd, order
