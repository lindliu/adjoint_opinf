...
def predict_and_plot(A_opt, H_opt, A_opinf_6, H_opinf_6, A_opinf_2, H_opinf_2, \
    ...
    if save_results:
        fig.savefig(f'./results_exact_rank/Results_exact_{name_suffix}.png')
...
def main(data_name, r, noise_level, step, smoother, pieces, reg_Frobenius=0, \
    ...
    if data_name == 'burgers':      
        ...
        Q_train, t_train, Q_valid, t_valid, Q_test, t_test, num_samples = \
            Q_reproj[:,:num_sam], t[:num_sam], Q_reproj[:,995:997], t[995:997], Q_reproj[:,997:], t[997:], t.shape[0]
    ...
    if save_results:
        np.savez(f"./results_exact_rank/theta_adjoint_with_opinf_{name_suffix}.npz", \
                 A_adjoint=A_opt, H_adjoint=H_opt, A_opinf=A_opinf, H_opinf=H_opinf)
        
        np.savez(f'./results_exact_rank/Predictions_{name_suffix}.npz', \
...
if __name__ == "__main__": 
    ...
    for opinf_use_val in [False]:
        ...
        for data_name in ['burgers']:
            ...
            for noise_level in [0]:
                ...
                if save_results:
                    per = f"{pertube_level:.0e}".split('e')[-1][-1]
                    np.savez(f"./results_exact_rank/error_{name_suffix}_per{per}.npz", 
                ...
                if save_results:
                    per = f"{pertube_level:.0e}".split('e')[-1][-1]
                    fig.savefig(f'./results_exact_rank/plot_exact_{name_suffix}_per{per}.png')
    ...
    for pertube in [0]:
        for r in r_list:
            theta_path = glob.glob(f'./results_exact_rank/theta_adjoint_*_best_dim{r}*per{pertube}.npz')[0]
